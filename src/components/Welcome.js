import React from 'react'

const Welcome = () => {
  return (
    <div>
      <h1>GO SHOP</h1>
      <p>Welcome to go shop.</p>
      <p>Create your own list for easy shopping.</p>
      <p>You can add and remove items to your list from other lists.</p>
      <p>At MY LIST you can see what is on your list at any time and remove items or mark them as bought or not bought.</p>
      <p>Once your happy with your list you're ready to shop.</p>
      <p>To start off press the LISTS button below</p>
    </div>
  )
}

export default Welcome
